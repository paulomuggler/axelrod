...
//	PaintSample paintSample = new PaintSample();
	...
	JRadioButton colorRepresentation = new JRadioButton(new AbstractAction() {
		public void actionPerformed(ActionEvent e) {
			SimulationApplet.this.switchCanvas(new CultureColorsCanvas(CANVAS_HEIGHT, sim.nw));
		}
	});
	@SuppressWarnings("serial")
	JRadioButton bordersRepresentation = new JRadioButton(new AbstractAction() {
		public void actionPerformed(ActionEvent e) {
			SimulationApplet.this.switchCanvas(new CultureBordersCanvas(CANVAS_HEIGHT, sim.nw));
		}
	});
...
//		controls.add(new JSeparator(SwingConstants.HORIZONTAL), "span 3, grow, wrap, gaptop 9, gapbottom 9");
		...
		wrapper.setPreferredSize(new Dimension(CANVAS_HEIGHT,CANVAS_HEIGHT));
		wrapper.add(canvas);
...
//					SimulationApplet.this.updateStroke();
//				}
//			}
//		};
//	}
...
//		this.pack();
	...
	protected void switchCanvas(final CultureCanvas cnv){
		final Container pane = SimulationApplet.this.getContentPane();
		if (canvas != null) {
			pane.remove(canvas);
			SwingUtilities.invokeLater(new Runnable() {
				@Override
				public void run() { 
//					SimulationApplet.this.pack();
					SimulationApplet.this.repaint();
					pane.repaint();
				}
			});
		}
		boolean toggled = false;
		if(SimulationApplet.this.sim.state.equals(CultureDisseminationSimulation.SimulationState.RUNNING)){
			SimulationApplet.this.toggle_simulation.actionPerformed(null);
			toggled = true;
		}
		canvas = cnv;
		SwingUtilities.invokeLater(new Runnable() {
